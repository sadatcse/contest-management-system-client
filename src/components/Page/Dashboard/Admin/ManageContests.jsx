...
const ManageContests = () => {
    ...
    const handleConfirm = async (contestId) => {
        try {
            await axiosSecure.put(`/contest/confirm/${contestId}`);
            Swal.fire({
                icon: 'success',
                title: 'Confirmed!',
                text: 'Contest confirmed successfully!',
                timer: 2000,
                timerProgressBar: true,
                showConfirmButton: false,
            });
    ...
    useEffect(() => {
        if (selectedContest) {
            setComment(selectedContest.comments || '');
        }
    }, [selectedContest]);
    ...
    const handleUpdateComment = async (contestId, comment) => {
        try {
            await axiosSecure.put(`/contest/update-comment/${contestId}`, { comments: comment });
    ...
    {contest.status === 'pending' ? (
        ...
        <span>Already Confirmed</span>
                                ...
                                <td className="border px-4 py-2">
                                    <button
                                        onClick={() => handleComments(contest)}
