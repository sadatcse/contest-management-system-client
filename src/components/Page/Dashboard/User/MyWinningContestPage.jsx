...
const MyWinningContestPage = () => {
    ...
    const { data: ContestsData = [], isLoading, isError, refetch } = useQuery({
        ...
        queryFn: async () => {
            try {
                const res = await axiosSecure.get(`/userole/get-win/${email}`);
    ...
    return (
        <div className="p-4">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-3xl font-bold">My Win Contest List</h2>
                <h2 className="text-3xl font-bold">Total Win Contests: {ContestsData.length}</h2>
            </div>
                ...
                <table className="min-w-full table-auto">
                    <thead>
                        <tr className="bg-gray-200">
                            ...
                            <th className="px-4 py-2">My submit link</th>
                    ...
                    <tbody>
                        {updateContestData().map((contest, index) => (
                            <tr key={contest._id} className={index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                                ...
                                <td className="border px-4 py-2">{contest.inputlink}</td>
